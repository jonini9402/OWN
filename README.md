...
### 1. 마이페이지 탭 전환 시 좋아요·북마크한 타인의 게시글 누락 문제
마이페이지 조회 쿼리가 본인이 작성한 게시물만 가져오도록 되어 있어, 좋아요·북마크한 게시물이 누락되는 문제를 확인하고 별도 조회 로직을 추가해 해결했습니다.
* **문제 상황**: 마이페이지를 [나의 운동일지 / 좋아요 / 북마크] 3개 탭으로 구성했으나, 초기 조회 시 작성자 식별자(`p.user_id = #{userId}`) 조건만 적용되어 타인이 작성한 글에 좋아요나 북마크를 등록한 데이터가 마이페이지 내에서 누락되는 현상 발생.
* **해결 방안**: 마이페이지 조회 API를 각 탭의 목적에 맞게 분리하고, MyBatis 매퍼(`PostMapper.xml`)에 관계 매핑 테이블을 활용한 별도 조회 쿼리를 구현했습니다.
  * `findLikedPosts`: `post` 테이블과 `user_post_like` 테이블을 **내부 조인(INNER JOIN)**하여 내가 좋아요를 누른 타인의 게시글 데이터를 정확히 추출.
  * `findBookmarkedPosts`: `user_post_bookmark` 테이블과의 **조인 및 조건 필터링**을 통해 북마크된 데이터를 누락 없이 매핑.
* **추가 성과**: 조회용 공통 SQL 조각(`selectPostDetails`) 내에 `EXISTS` 서브쿼리를 활용하여, 마이페이지 목록 렌더링 시에도 해당 게시글들의 좋아요/북마크 활성화 상태(`isLiked`, `isBookmarked`)가 실시간으로 유지되도록 데이터 정합성을 확보했습니다.

### 2. 프론트엔드 내 외부 AI API 자격 증명(API Key) 노출 취약점 개선
* **문제 상황**: Vue.js 3 기반 AI 챗봇 모달(`ChatBotModal.vue`) 구현 과정에서, GPT 모델 호출을 위한 자격 증명 키(`GMS_KEY`)를 컴포넌트 내부에 하드코딩한 채 퍼블릭 레포지토리에 커밋하여 민감한 API 키가 외부로 노출되는 보안 취약점 발생.
* **해결 방안**: 자격 증명 노출 즉시 기존 키를 만료 및 재발급하여 권한 오용을 차단하고, 프론트엔드 빌드 환경에 맞춘 보안 고도화를 진행했습니다.
  * **환경 변수 격리**: 소스 코드 내부의 자격 증명 문자열을 완전히 제거하고, Vite 환경 변수 파일(`.env`)로 분리 관리하도록 구조를 변경.
  * **Git 추적 원천 차단**: `.gitignore`에 해당 환경 변수 파일을 등록하여 코드 저장소 업로드 시 민감 정보가 포함되지 않도록 관리 프로세스 정립.
* **기술적 인지**: 본 트러블슈팅을 통해 브라우저 측(Client-side) 환경 변수는 빌드 시점에 노출될 위험이 있음을 인지했으며, 향후 실서비스 환경에서는 보안성 향상을 위해 API 키 주입 및 외부 통신을 백엔드 프록시 서버(BFF 패턴 등)를 거치도록 설계해야 한다는 보안 인식을 정립하는 계기가 되었습니다.
